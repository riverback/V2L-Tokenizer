...
def main(args):
    ...
    if os.path.exists(os.path.join(args.output_dir, "%d-k_%d-way_%d-shot_%d-repeat_%d-induction.npy"%(args.token_num, args.way, args.shot, args.repeat, args.induction))):
        ...
        dataset = MiniImageNetDataset(
            data_root=args.mini_imagenet_path, image_size=args.image_size, time=time, 
            clip_texts=clip_texts, clip_codebook=clip_codebook, model_path=args.llama_model_path, 
            vision_tokenizer=model, text_token_embedding=generator.model.tok_embeddings, 
            way=args.way, shot=args.shot, device=device, induction=args.induction
        )
    else:
        dataset = MiniImageNetDataset(
            data_root=args.mini_imagenet_path, image_size=args.image_size, time=0, 
            clip_texts=clip_texts, clip_codebook=clip_codebook, model_path=args.llama_model_path, 
            vision_tokenizer=model, text_token_embedding=generator.model.tok_embeddings, 
            way=args.way, shot=args.shot, device=device, induction=args.induction
        )
